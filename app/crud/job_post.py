...
def get_job_posts_by_id(job_post_id:int,db: Session):
    return db.query(models.Job_post).filter(models.Job_post.id == job_post_id).filter(models.Job_post.status != 0).first()
...
def create_job_post(db: Session, job_post: schemas.Job_post_Create, username: str, id_company: int):
    ...
    db_job_post.expired_time = db_job_post.create_time
    db.add(db_job_post)
    db.commit()
